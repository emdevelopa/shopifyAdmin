var hoverElement = document.getElementById("checkbox");
var clicked = document.getElementById("clicked");

function handleHover() {
  // Your hover logic here

  // Set a timeout to remove the hover effect after 1000 milliseconds (1 second)
  setTimeout(() => {
    hoverElement.removeEventListener("mouseover", handleHover);
    console.log("Hover effect removed");
  }, 1000);
}
hoverElement.addEventListener("mouseover", function () {
  // console.log('Mouse over the element');
  hoverElement.innerHTML = ` <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
        <circle cx="12" cy="12" r="10" stroke="#8A8A8A" stroke-width="2.08333" stroke-linecap="round"
...
const svgs = [
  `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
        <circle cx="12" cy="12" r="10" fill="#D9D9D9" stroke="#8A8A8A" stroke-width="2.08333" stroke-linecap="round"
            stroke-linejoin="round" />
    </svg>`,
...
hoverElement.addEventListener("mousedown", function () {
  ...
  function displayCurrentSvg() {
    hoverElement.innerHTML = svgs[currentSvgIndex];
  }
  ...
  // Attach the click event to the initial SVG
  hoverElement.addEventListener("click", () => {
    isLooping = true;
    loop();
    setTimeout(stopLoop, svgs.length * 200); // Stop the loop after all SVGs have been displayed
  });
  ...
  setTimeout(() => {
    ...
    hoverElement.innerHTML = `<svg class="checkedbox pointer" tabindex="0" id="dropdown-checkbox-checked" width="24" height="24"
    ...
    // hoverElement.removeAttribute("id")
  }, 1000);
});
